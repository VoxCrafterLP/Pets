...
public class WolfPet extends CustomPet {
    ...
    public Entity spawn() {
        Wolf wolf = (Wolf) this.petData.getLocation().getWorld().spawnEntity(this.petData.getLocation(), EntityType.WOLF);
        wolf.setAdult();
        wolf.setCustomName(this.petData.getDisplayMame());
    ...
    public void onTick() {
        Bukkit.getOnlinePlayers().forEach(players -> {
            if(this.entity.getLocation().distance(players.getLocation()) <= Pets.getInstance().getPetsConfig().getPetRadius()) {
                players.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 100, 0, true, false));
            }
        });
    ...
    public void updateSitting() {
        ...
        ((Wolf) this.entity).setSitting(this.petData.isSitting());
    }

    private void followPlayer() {
        ((Wolf) this.entity).setSitting(false);
