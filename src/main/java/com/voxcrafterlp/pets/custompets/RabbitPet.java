...
public class RabbitPet extends CustomPet {
    ...
    public Entity spawn() {
        Rabbit rabbit = (Rabbit) this.petData.getLocation().getWorld().spawnEntity(this.petData.getLocation(), EntityType.RABBIT);
        rabbit.setAdult();
        rabbit.setCustomName(this.petData.getDisplayMame());
    ...
    public void onTick() {
        Bukkit.getOnlinePlayers().forEach(players -> {
            if(this.entity.getLocation().distance(players.getLocation()) <= Pets.getInstance().getPetsConfig().getPetRadius()) {
                players.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 100, 0, true, false));
            }
        });
